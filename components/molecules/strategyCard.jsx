...
function StrategyCard({ data }) {
  const cardTheme = data.status == "Running" ? "#00D816" : data.status === "Finished" ? "#FF4646" : "#4649FF";
  const profit = (data.chartData[9].y + 1) / (data.chartData[0].y + 1) - 1;
  return (
    <Pressable onPress={() => {}} style={style.card}>
      <View
        style={{
      ...
      <View style={style.cardContent}>
        ...
        <View>
          <Text style={{ fontWeight: "bold" }}>Rp.{(data.chartData[9].y * 1000).toFixed(0)}</Text>
          <Text
            style={{
              color: profit > 0 ? "#44bd32" : "#FF4646",
            }}
